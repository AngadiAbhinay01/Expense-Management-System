...
def get_analytics(date_range: DateRange):
    """
    Category-wise analytics for a date range
    """
    data = db_helper.fetch_expense_summary(date_range.start_date, date_range.end_date)
    if data is None:
        raise HTTPException(status_code=500, detail="Failed to retrieve expense summary from the database.")
...
def get_analytics_by_month(date_range: DateRange):
    """
    Monthly analytics for a date range
    """
    ...
    # Convert data to structured dictionary
    result = {}
    for row in data:
        month_label = f"{row['month_name']} ({row['expense_month']})"
        result[month_label] = row['total']
